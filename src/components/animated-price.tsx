...
export function AnimatedPrice({ price }: AnimatedPriceProps) {
  ...
  useEffect(() => {
    if (isDiscounted) {
      ...
      }, 1500); // Should match transition duration
      return () => clearTimeout(timer);
    } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-24">
      <div className="relative h-20 flex items-center justify-center w-48">
        ...
        {showInitialPrice && (
          <span
            className={`font-headline font-extrabold text-foreground transition-all duration-[1500ms] ease-in-out ${
              isDiscounted
                ? "opacity-0 scale-50"
        ...
        <div
          className={`absolute flex items-end justify-center gap-x-2 sm:gap-x-3 transition-all duration-[1500ms] ease-in-out ${
            isDiscounted
      ...
      <p
        className={`mt-2 text-sm font-medium transition-colors duration-[1500ms] ${
